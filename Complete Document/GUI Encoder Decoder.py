...
def on_encrypt_with_keys_clicked():
    plaintext = plaintext_entry.get()
    alphabet = string.ascii_letters
    key1 = key1_entry.get()
    key2 = key2_entry.get()
...
encrypt_with_keys_text_entry = tk.Entry(root)
...
encrypt_with_keys_button = tk.Button(root, text="Encrypt with Keys", command=on_encrypt_with_keys_clicked)
...
encrypt_with_keys_button.grid(row=5, column=1)
decrypt_button.grid(row=5, column=2)
